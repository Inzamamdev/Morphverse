...
export default function ButtonsGrid({ setShape, setStage }: Props) {
    return (
        <div className="w-full h-20 border-2 bg-slate-500 flex justify-evenly items-center  select-none">
            <button className="px-4 py-2 bg-green-500 text-white rounded" onClick={() => setShape('square')}>Triangle</button>
            ...
            <button className="px-4 py-2 bg-purple-500 text-white rounded" onClick={() => setStage(1)}>Stage 1</button>
        </div>
    )
}